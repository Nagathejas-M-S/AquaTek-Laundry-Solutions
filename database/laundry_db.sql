...
CREATE PROCEDURE GetLaundryAmountForToday()
BEGIN
...
CREATE TRIGGER before_insert_laundry_category
BEFORE INSERT ON laundry_categories
FOR EACH ROW
BEGIN
    -- You can log the insert action, or apply constraints here
    -- Example: Just ensuring the name and price are set
    IF NEW.name IS NULL OR NEW.price IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Name and Price cannot be NULL';
    END IF;
END$$

-- Trigger for UPDATE
CREATE TRIGGER before_update_laundry_category
BEFORE UPDATE ON laundry_categories
FOR EACH ROW
BEGIN
    -- Example: Ensure that price is always greater than zero
    IF NEW.price <= 0 THEN
...
-- Trigger for DELETE
CREATE TRIGGER before_delete_laundry_category
BEFORE DELETE ON laundry_categories
FOR EACH ROW
BEGIN
    -- You can log or restrict deletes, for example
    -- Example: Prevent deletion of categories with associated orders
    -- SELECT count(*) INTO @order_count FROM orders WHERE category_id = OLD.id;
    -- IF @order_count > 0 THEN
    --     SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot delete category with associated orders';
    -- END IF;
END$$
